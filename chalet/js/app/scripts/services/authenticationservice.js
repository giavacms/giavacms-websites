...
angular.module('jsApp')
    ...
    .factory('AuthenticationService', ['$http', '$interval', '$log', '$q', '$rootScope', 'jwtHelper', 'APP_PROPERTIES', 'StorageService',
        function ($http, $interval, $log, $q, $rootScope, jwtHelper, APP_PROPERTIES, StorageService) {
            ...
            var service = {
                ...
                isLogged: function () {
                    ...
                    return StorageService.get('token').then(function (storedToken) {
                        if (storedToken && storedToken.length > 0) {
                            token = storedToken[0];
                            if (token && token instanceof String && token.indexOf(".") > -1) {
                ...
                login: function (phone) {
                    var url = '//' + host + '/api/v1/accounts/login';
                    $http.post(url, {
                        phone: phone
                    }).success(function (data, status, headers, config) {
                        // this callback will be called asynchronously
                ...
                confirm: function () {
                    var url = '//' + host + '/api/v1/accounts/login/' + uuid + '/token';
